"""
Application: bishop_bagging_kNN.py

Training Dataset: bishop_synthetic.csv
    
Description: Use bagging and the kNN classifier to classify the NBA dataset

How to Run: python Applications/bishop_bagging_kNN.py (from root ML1050 directory)
"""
...
def main():
    ...
    outfile = open('bishop_bagging_kNN.pickle','wb')
    kNNModel = kNN(bestK, distanceFunction)
    bagger = Bagging(kNNModel,20)
    bagger.train(trainingSet)
    cPickle.dump(bagger,outfile)
    outfile.close() 
