"""
Application: latLong_DecisionTree.py

Training Dataset: usLatLong.csv
    
Description: This application will compare an X node decision tree versus Y one-node 
decision trees using AdaBoost. It uses the dataset of US cities, classifying between 
Colorado and NonColorado cities.  Will get different validation and training errors each run
because uses kfold and builds a new tree each run. Hardcoded the best theta, which 
was found during cross validation.

How to Run: python Applications/latLong_DecisionTree.py (from root ML1050 directory)
"""
...
def main():
    ...
    dataSet = TrainingSet('../Datasets/usLatLong.csv')
