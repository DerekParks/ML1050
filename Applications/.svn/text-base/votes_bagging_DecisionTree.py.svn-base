"""
Application: votes_bagging_DecisionTree.py

Training Dataset: votes-train0.csv
    
Description: Use a Decision Tree to classify the votes (Democrat/Republican) dataset.
Will get different validation and training errors each run because uses kfold and
builds a new tree each run. Hardcoded the best theta, which was found during cross
validation.

How to Run: python Applications/votes_bagging_DecisionTree.py
...
def main():
    ...
    #compute test set error if user provided a testset
    testError = 0.0
    if len(testSet) > 0:
