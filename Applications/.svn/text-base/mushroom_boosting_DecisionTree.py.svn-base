"""
Application: mushroom_boosting_DecisionTree.py

Training Dataset: mushroom.csv
    
Description: Application using boosting and the decision tree classifier on the
mushroom dataset.

How to Run: python Applications/mushroom_boosting_DecisionTree.py
(from root ML1050 directory)
"""


from ML1050.Classifiers.DecisionTree import DecisionTree
from ML1050.Classifiers import AdaBoost
...
def main(argv):
    ...
    for training, validation in kFold(dataSet, k=numFolds):
        DTmodel = DecisionTree(theta, maxDepth=1)
        AdaModel = AdaBoost(DTmodel,M=100)
        AdaModel.train(dataSet)
        
        validationError = 0.0
        for example in validation:       
            if AdaModel.test(example) != \
        ...
        for example in training:
            if AdaModel.test(example) != example.label: trainingError += 1
        #compute training error
        trainingError = float(trainingError)/len(training)
        avgTrainingError += trainingError
    DatasetError = 0    
    for example in dataSet:
        if AdaModel.test(example) != example.label: DatasetError += 1
    ...
    if len(testSet) > 0:
        for example in testSet:
            if AdaModel.test(example) != example.label: testError += 1
