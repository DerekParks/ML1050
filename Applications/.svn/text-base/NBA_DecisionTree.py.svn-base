"""
Application: NBA_DecisionTree.py

Training Dataset: NBA_TRAIN.csv
    
Description: This application will create an X node decision tree. It uses the NBA dataset
which will classify player positions based on height and weight. This application was
created to test the ability of the decision tree classifier to handle continuous variables
in suboptimal conditions (variables are semi-discrete). Will get different validation and 
training errors each run because uses kfold and builds a new tree each run.

How to Run: python Applications/NBA_DecisionTree.py (from root ML1050 directory)
"""
